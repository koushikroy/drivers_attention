...
with mp_face_mesh.FaceMesh(
  ...
  while cap.isOpened():
    ...
    distance_left = dist = np.sqrt(np.sum(np.square(
      np.array((face_landmarks.landmark[left_corners[0]].x, face_landmarks.landmark[left_corners[0]].y))-
      np.array((face_landmarks.landmark[left_corners[1]].x, face_landmarks.landmark[left_corners[1]].y))
      )))
